...
int test_deplacement_pion(int ligne1, int colonne1, int ligne2, int colonne2, damier *d) {
    ...
    /*if((d->pion_dm[ligne2][colonne2].contenu_case) == dames_adverses)
        return 0;  cas traité plus haut 
        */
    return 1;
}
int deplacement_pion(int ligne1, int colonne1, int ligne2, int colonne2, damier *d) { // pas de notation de Manoury
	...
	/*int man_from = d->pion_dm[ligne1][colonne1].manoury;
	int man_to = d->pion_dm[ligne2][colonne2].manoury;*/
	// On envoie au joueur adverse (man_from,man_to) le parcours du pion (partie réseau).
    if(d->rb == 1 && ligne2 == 0) {
...
int deplacement_pion_adverse(int man1, int man2, damier *d) { // fonction de déplacement simple (pas de prise).
    ...
    /*if(d->ad == 1 && d->pion_dm[i1][j1].contenu_case == PION_BLANC) {
        printf("gggggggggggggggggggg\n");
        if(i2 >= i1)
            return 0; // pion blanc ne peut pas retourner en arrière 
    }
    if(d->ad == 0 && d->pion_dm[i1][j1].contenu_case == PION_NOIR) {
        printf("hhhhhhhhhhhhhhhhhhhhhhhhhh\n");
        if(i2 <= i1)
            return 0; // pion noir ne peut pas retourner en arrière
    }*/
    ...
    //printf("kkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk\n");
...
int prise_adverse(int man1, int man2, damier *d) {
	/* On récupère les coordonnées du pion avant et après le coup */
    if(man1 == man2)
        return 0;
...
liste *rafle_max(damier *d, int lg, int cl) {
    ...
    element *er = ls_max->premier;
    while(er != NULL) {
        cpl *cr = malloc(sizeof(cpl));
        cr->i = er->i;
        cr->j = er->j;
        printf("je suis dans fct rafle max %u \n",coord_to_man(cr));
        er = er->suivant;
    }
    return ls_max; 
}
